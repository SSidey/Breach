...
func test_reaching_the_core_wins_immediately() -> void:
	## Decision 16: no longer gated on the Hero Party being defeated first - found via
	## manual playtest, a wave that reached the Core before the Hero Party even
	## arrived just kept marching, since the old gate silently discarded the capture.
...
func test_losing_the_entire_force_loses() -> void:
	var watcher := ScriptedBeatWatcher.new(CORE_INDEX)
	monitor_signals(SimEvents, false)
